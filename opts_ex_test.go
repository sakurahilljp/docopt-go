...
func TestBindEx_OptsBind(t *testing.T) {
	...
	for i, c := range []struct {
		...
		if err := v.BindEx(&result); err != nil {
...
func TestBindEx_BindSimpleStruct(t *testing.T) {
	...
	if err := opts.BindEx(&opt); err != nil {
		t.Fatal(err)
	}
	if opt.Number != 123 {
		t.Fail()
...
func TestBindEx_BindingMultiLevelAnonymousStruct(t *testing.T) {
	...
	if err := opts.BindEx(&actual); err != nil {
...
func TestBindEx_BindingMultipleTaggedFields(t *testing.T) {
	...
	if err := optsShort.BindEx(&actualShort); err != nil {
...
func TestBindEx_BindingOptionalArguments(t *testing.T) {
	...
	if err := optsMissing.BindEx(&actualMissing); err != nil {
	...
	if err := optsPresent.BindEx(&actualPresent); err != nil {
...
func TestBindEx_BindToPointerField(t *testing.T) {
	type PointerConfig struct {
		...
		Id     *int     `docopt:"--id"`
		Weight *float64 `docopt:"--weight"`
	}

	var testParser = &Parser{HelpHandler: NoHelpHandler, SkipHelpFlags: true}
	usage := `Usage: mytool [--name=NAME] [--id=ID] [--weight=WEIGHT]`

	{
		argv := []string{"--name", "test", "--id", "123", "--weight", "4.5"}
	...
	{
		argv := []string{"--name", "test", "--weight", "4.5"}
